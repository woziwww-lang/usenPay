  ...
  return (
    <section className="rounded-lg border border-line bg-white shadow-panel">
      <div className="flex flex-col gap-3 border-b border-line p-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-base font-semibold text-ink">Live order board</h2>
      ...
      <div className="divide-y divide-line">
        {visible.map((order) => (
          <article key={order.id} className="grid gap-3 p-4 md:grid-cols-[1fr_140px_120px] md:items-center">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="font-semibold text-ink">{order.id}</h3>
                <StatusBadge value={order.status} />
              </div>
              <p className="mt-1 text-sm text-slate-600">{order.items.join(" / ")}</p>
            ...
            <p className="font-semibold text-ink">{currency.format(order.amount)}</p>
